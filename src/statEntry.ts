...
async function updateStat(updateIncome: number, updateOrder: number, updateMeal: Array<mealUpdate>) {
    ...
    await saleStat.update({
        ...
        OrderNum: admin.firestore.FieldValue.increment(updateOrder),
    });

    updateMeal.forEach(async e => await db.collection('mealStat').doc(e.Name).update({ Quantity: admin.firestore.FieldValue.increment(e.Quantity) }))
